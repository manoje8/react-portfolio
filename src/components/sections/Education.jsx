  ...
  useGSAP(() => {
    ...
    rows.forEach((row, index) => {
      ...
      revealTl
        ...
        .to(
          tags,
          { opacity: 1, y: 0, duration: 0.4, stagger: 0.03, ease: "back.out(1.7)" },
          "<0.1"
        );
    ...
    certTl
      ...
      .from(
        ".cert-row",
        { opacity: 0, x: 16, duration: 0.5, stagger: 0.1, ease: "power2.out" },
        "-=0.4"
      );
          ...
          <div className="edu-timeline relative space-y-14 pl-6 md:pl-8">
            ...
            {education?.map((data, id) => (
              <article key={id} className="edu-row group relative space-y-3">
                ...
                <p className="edu-subtitle text-lg text-zinc-400 italic font-serif">
                  {data.subtitle}
                </p>
        ...
        <div className="certificate-column lg:col-span-5 lg:sticky lg:top-24 h-fit space-y-12">
          ...
          <div className="divide-y divide-zinc-900 border-b border-zinc-900">
            {certificates?.map((certificate, id) => (
              <div
                key={id}
                className="cert-row group/cert py-4 flex items-center justify-between transition-colors duration-300"
              >
                <div className="flex items-center gap-4 cursor-pointer">
                  <span>{id + 1}.</span>
                  <span className="text-lg font-medium text-zinc-300 group-hover/cert:text-white transition-colors duration-300">
                    {certificate}
                  </span>
                </div>
                <span className="text-[10px] font-mono text-zinc-600 tracking-wider uppercase opacity-0 group-hover/cert:opacity-100 transition-all duration-300 translate-x-2 group-hover/cert:translate-x-0">
                  Verified
                </span>
              </div>
            ))}
